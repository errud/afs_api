...
AS
BEGIN
	UPDATE Supplier SET Company = @company, Logo = @logo, SectorId = @secId, ServiceId = @serId, Membership = 0, Contact = @contact, Phone = @phone, Email = @email, [Url] = @url,  [Address] = @address, City = @city,	Country = @ctry, AdditInfo = @ainfo, [Status] = 1, Created = Getdate()
	WHERE Id = @id and UserId = @userid
END
GO
...
EXEC AddAddress 4, 'Kolo','Touré','Quartier Plateaux','Abidjan','',225,'Ivory Coast'
